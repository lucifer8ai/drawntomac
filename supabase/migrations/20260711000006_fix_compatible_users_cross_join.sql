-- Migration: Fix get_compatible_users CROSS JOIN → LEFT JOIN for empty shared CTE
-- When shared has 0 rows (no compatible users), CROSS JOIN produces 0 rows,
-- killing current_user_total. Flip driver table so user_total always flows through.
