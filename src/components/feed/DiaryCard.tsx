    ...
    >
      <div className="flex items-center gap-3 border-b px-4 py-3">
        {data.albumArtUrl ? (
          ...
          <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-lg bg-muted text-xs text-muted-foreground">
            —
          </div>
        )}

        <div className="min-w-0 flex-1">
          <div className="truncate text-base font-semibold font-serif text-foreground" title={data.songTitle}>
            {data.songTitle}
          </div>
          {data.artistName && (
