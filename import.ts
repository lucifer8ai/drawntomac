...
export const Route = createFileRoute("/api/spotify/import")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let raw: unknown;
        ...
        if (body.artistSpotifyId) {
          const { data: artist, error: artistErr } = await supabaseAdmin
            .from("artists")
            .upsert(
              {
                name: body.artistName,
                slug: slugifyBase(body.artistName).slice(0, 80) || body.artistSpotifyId,
                spotify_id: body.artistSpotifyId,
              },
            ...
            .single();
          if (artistErr) {
            console.error("[spotify import] artist upsert", artistErr);
            return Response.json({ error: artistErr.message }, { status: 500 });
          }
          artistId = artist?.id ?? null;
        }

        const slug = body.slug ?? slugifySong(body.title, body.spotifyId);
        ...
        const { error: songErr } = await supabaseAdmin
          .from("songs")
          .upsert(songRow, { onConflict: "spotify_id" });
        if (songErr) {
          console.error("[spotify import] song upsert", songErr);
